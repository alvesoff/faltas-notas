...
export async function GET(request: NextRequest) {
  try {
    ...
    switch (tipo) {
      ...
      case 'turmas':
        query = `
          SELECT DISTINCT 
            ...
            c.Turma as turma
          FROM (
            SELECT DISTINCT Mat, AnoLetivo FROM NotasFaltas WHERE AnoLetivo = ?
            UNION
      ...
      case 'bimestres':
        query = 'SELECT DISTINCT Bim FROM NotasFaltas WHERE AnoLetivo = ? ORDER BY Bim';
        params = [anoLetivo];
        break;
    ...
    return NextResponse.json({
      success: true,
      data: results,
      tipo: 'relatorio-notas'
    });

  } catch (error) {
    console.error('Erro na API de filtros - Relatório de Notas:', error);
    return NextResponse.json(
      { 
        success: false, 
        error: 'Erro interno do servidor - Filtros Relatório de Notas',
