...
interface DadosRelatorio {
  ...
  // Campos específicos para faltas resumidas
...
interface TabelaRelatoriosProps {
  ...
  carregando: boolean;
  tipo: 'faltas' | 'notas';
  tipoFalta?: 'detalhadas' | 'resumidas'; // novo prop
}

type CampoOrdenacao = keyof DadosRelatorio;

export default function TabelaRelatorios({ dados, carregando, tipo, tipoFalta = 'detalhadas' }: TabelaRelatoriosProps) {
  ...
  const getStatusFaltas = (percentual: number | null | undefined) => {
    const valor = typeof percentual === 'number' ? percentual : 0;
    if (valor >= 25) return { cor: 'text-red-600', icone: AlertTriangle, texto: 'Crítico' };
    if (valor >= 15) return { cor: 'text-yellow-600', icone: AlertTriangle, texto: 'Atenção' };
    return { cor: 'text-green-600', icone: CheckCircle, texto: 'Normal' };
  };
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <h3 className="text-lg font-semibold text-blue-900">
              {tipo === 'faltas' ? 'Relatório de Faltas' : 'Relatório de Notas'}
            </h3>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="table-header">
            <tr>
              ...
              {/* Colunas específicas para Faltas Resumidas */}
              {tipo === 'faltas' && tipoFalta === 'resumidas' && (
                <>
                  <th 
                    className="table-cell cursor-pointer hover:bg-gray-100 select-none"
              ...
              {/* Colunas para Faltas Detalhadas e Notas */}
              {(tipo === 'notas' || (tipo === 'faltas' && tipoFalta === 'detalhadas')) && (
                <>
                  <th 
                    className="table-cell cursor-pointer hover:bg-gray-100 select-none"
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {dadosPaginados.map((item, index) => {
              const statusFaltas = getStatusFaltas(item.percentualFaltas);
              const statusNota = tipo === 'notas' ? getStatusNota(item.nota) : null;
              const StatusIcon = tipo === 'faltas' ? statusFaltas.icone : statusNota?.icone || CheckCircle;
              const statusCor = tipo === 'faltas' ? statusFaltas.cor : statusNota?.cor || 'text-gray-600';
              const statusTexto = tipo === 'faltas' ? statusFaltas.texto : statusNota?.texto || 'N/A';

              return (
                <tr key={`${item.matricula}-${item.disciplina || 'resumo'}-${item.bimestre || 'geral'}-${index}`} className="table-row">
                    ...
                    <>
                      ...
                      {tipo === 'notas' && (
                        <td className="table-cell text-center font-semibold">
                          <span className={`${statusNota?.cor || 'text-gray-900'}`}>
                      ...
                      <td className="table-cell">
                        <div className={`flex items-center gap-1 ${statusCor}`}>
                          <StatusIcon className="w-4 h-4" />
                          <span className="text-xs font-medium">{statusTexto}</span>
                        </div>
                      </td>
                    </>
                  )}
